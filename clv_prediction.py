...
def x_y():
    global X
    global Y
    X = enc_data.drop(columns=["Customer Lifetime Value"])
    Y = enc_data["Customer Lifetime Value"]
...
variables=['State', 'Response', 'Coverage',
                       ...
                       'VehiclePrice','Monthly Premium Auto','Product Name','AgeOfVehicle','Customer Lifetime Value']
...
for i in range(0,len(MAPE_df.T)):
    if MAPE_df.T.iloc[i,0]==MAPE_df.T[0].min():
        pickle.dump(pipelines_list[i], open('clv_prediction_model.pkl', 'wb'))